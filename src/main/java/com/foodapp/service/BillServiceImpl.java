...
public class BillServiceImpl implements BillService{
	...
	public Bill addBill(Bill bill) throws BillException {
		if (bill.getOrder() == null || bill.getOrder().getOrderId() == null) {
			throw new BillException("Order ID must not be null!");
		}

		Optional<Bill> existingBill = billDAO.findByOrder_OrderId(bill.getOrder().getOrderId());
		if (existingBill.isPresent()) {
			throw new BillException("Bill already exists for this order.");
		}
